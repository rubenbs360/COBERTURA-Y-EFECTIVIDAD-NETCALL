...
OUTPUT_DIR = r"c:\Users\USUARIO\OneDrive\Escritorio\RUBEN DOC\NETCALL\data"
...
def process_data():
    print("Loading CSV Outbound Netcall...")
    # Read the large CSV file
    df_csv = pd.read_csv(CSV_PATH, encoding='utf-8', encoding_errors='ignore')
    
    print("Loading Excel Store Directory...")
    xl = pd.ExcelFile(XLSX_PATH)
    
    # 1. Process Advisors (Lima + Prov)
    print("Processing Advisors...")
    df_adv_lima = xl.parse("ASESORES LIMA")
    df_adv_prov = xl.parse("ASESORES PROV")
    ...
    advisors_by_store = {}
    active_advs = df_adv[df_adv['ESTADO'] == 'APROBADO'] # Wait, in the Excel it was HC_BASE=APROBADO? Let's check what values ESTADO column has.
    # Let's check ESTADO values
    print("Advisor ESTADO values:", df_adv['ESTADO'].value_counts().to_dict())
    
    # Let's take all advisors if ESTADO is empty or not INACTIVO
    active_advs = df_adv[~df_adv['ESTADO'].str.contains('INACTIVO|BAJA', na=False, case=False)]
    ...
    # 2. Process Stores (LIMA SUP + PROV SUP)
    print("Processing Stores...")
    df_sup_lima = xl.parse("LIMA SUP")
    df_sup_prov = xl.parse("PROV SUP")
    
    # Standardize column names for LIMA SUP
    df_sup_lima = df_sup_lima.rename(columns={'# PDV': 'ID_PDV', 'NOMBRE PDV': 'PDV', 'DEPARTAMENTO2': 'DEPARTAMENTO_2'})
    # Standardize column names for PROV SUP
    ...
    dept_stats = []
    # Replace NaN department with 'No especificado'
    df_csv['FRM_Departamento_de_entrega'] = df_csv['FRM_Departamento_de_entrega'].fillna("No especificado")
    
    for dept, group in df_csv.groupby('FRM_Departamento_de_entrega'):
        d_total = len(group)
        ...
        # Dispatch breakdown with outcomes
        dispatch_breakdown = {}
    ...
    with open(os.path.join(OUTPUT_DIR, "stores.json"), "w", encoding="utf-8") as f:
        json.dump(consolidated_stores, f, ensure_ascii=False, indent=2)
        
    with open(os.path.join(OUTPUT_DIR, "advisors.json"), "w", encoding="utf-8") as f:
