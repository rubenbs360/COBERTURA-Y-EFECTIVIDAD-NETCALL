  ...
  geoJsonLayer = L.geoJSON(coberturaData, {
    ...
    onEachFeature: function (feature, layer) {
      ...
      layer.on('click', function (e) {
        ...
        const deptName = props.departamento;
...
function updateMapMarkers() {
  ...
  storesData.forEach(store => {
    ...
    if (store.latitud && store.longitud) {
      if (selectedDept !== "todos" && store.departamento.toLowerCase() !== selectedDept) {
        return;
      }
...
// Populate the Department select element in Map Controls
function populateDeptSelect() {
  ...
  // Extract unique departments from data and sort
...
function handleMapFiltersChange() {
  updateRegionalPerformance();
  updateMapMarkers();
  ...
  const coord = deptCoordinates[selectVal] || deptCoordinates["todos"];
